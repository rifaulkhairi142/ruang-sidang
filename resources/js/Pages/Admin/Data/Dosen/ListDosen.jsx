...
const ListDosen = ({ base_url, message, props }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get(
                `${base_url}/api/admin/data/dosen?page=${currentPage}&search_key=${searchKey}`
            );
    ...
    const deleteRecords = async (id) => {
        ...
        try {
            const response = await axios.post(
                `${base_url}/api/admin/data/dosen/delete/${id}`
            );
            fetchData();
        } catch (err) {
            setError(err.response?.data?.message || "Something went wrong.");
    ...
    return (
        <AdminLayout>
            <Head title="List Dosen" />
            <LocalizationProvider dateAdapter={AdapterDayjs}>
                <section className="dark:bg-gray-900 sm:p-5">
                    <div className="mx-auto ">
                        <div className="flex justify-between items-center py-3">
                            <h2 className="text-2xl font-semibold">Dosen</h2>
                            <ul className="flex gap-x-3">
                                <li className="cursor-pointer">Dashboard</li>
                                <li>{">"}</li>
                                <li className="text-primary2-600">Dosen</li>
                            </ul>
                        </div>
